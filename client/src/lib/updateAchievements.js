...
export async function updateAllUserAchievements() {
  try {
    console.log('Создание достижений для всех пользователей...');
    ...
    // Для каждого пользователя создаем достижения
    for (const user of users) {
      console.log(`Создаем достижения для пользователя: ${user.username || user.firstName} (${user.telegramId})`);
      ...
      try {
        ...
        console.log(`Достижения для пользователя ${user.username || user.firstName} успешно созданы`);
        updatedUsersCount++;
      } catch (userError) {
        console.error(`Ошибка при создании достижений для пользователя ${user.username || user.firstName}:`, userError);
      }
    }
    
    const result = {
      success: true,
      message: `Достижения успешно созданы для ${updatedUsersCount} пользователей из ${users.length}`
    };
