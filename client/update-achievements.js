// Скрипт для обновления достижений у всех пользователей
// Запуск: node update-achievements.js
...
async function updateAchievements() {
  try {
    console.log('Обновление достижений для всех пользователей...');
    ...
    // Для каждого пользователя обновляем достижения
    for (const user of users) {
      console.log(`Обновляем достижения для пользователя: ${user.username || user.firstName} (${user.telegramId})`);
      ...
      try {
        ...
        console.error(`Ошибка при обновлении достижений для пользователя ${user.username || user.firstName}:`, userError);
      }
    }
    
    console.log(`Все достижения успешно обновлены! Обновлено пользователей: ${updatedUsersCount} из ${users.length}`);
  } catch (error) {
